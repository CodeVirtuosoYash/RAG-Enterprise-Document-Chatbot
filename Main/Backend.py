...
#Bert set up for creating embeddings
...
def purge_stale_metadata():
    ...
    for key in metadata_db:
        try:
            parts = key.split("/")      #Split the key into org, dept, and filename
            if len(parts) != 3:
                continue
            org, dept, filename = parts
            prefix = f"rag-project-bucket-01/test-output/{org}/{dept}/{filename}/"
            resp = s3.list_objects_v2(Bucket=S3_BUCKET, Prefix=prefix) #Check if the file exists in S3
            if "Contents" not in resp or not resp["Contents"]:
                keys_to_delete.append(key)
        except Exception as e:
            print(f"[Startup Check Error] Failed on key {key}: {e}")
    
    #checks for any files that are not in S3 and removes them from the metadata database
...
#Check if the Index is build or not
def ensure_index_built(prefix: str) -> bool:
    global faiss_index
    if faiss_index is not None and faiss_index.ntotal > 0:
        return True
    try:
        reload_index_s3(prefix)     #If index not build try to create one
...
#retrive similar documents based on Eulidean distance from the query embedding
...
#Prompt that will be send to the LLM
...
#THe promt is send to the LLM along with context and query
...
#Attempt building Index upon login
@app.post("/")
def auto_build_index(profile: dict):
    ...
    return {
        "message": "Index is ready" if built else "Failed to build index",      #if no contents in the relevent path then Index Failed to build will be returned 
        "index_built": built,
...
def process_file(data: UploadRequest) -> List[str]:
    def extract_images_from_zip(file_path: str, zip_prefix: str, output_dir: str) -> List[str]:
        #Checks if any images are present in the File being uploaded and stores them if any images are found.
        images = []
        try:
    ...
    #Describing the path for where to store in S3
    ...
    #If same file exists in same path, skip uploading it 
...
def get_index_status(org: str, dept: str, base_output_dir: str = "rag-project-bucket-01/test-output"):
    prefix = f"{base_output_dir}/{org}/{dept}"
...
#Revives the query from frontend and Genrates relevent answer
@app.post("/Answer")
def answer_question(data: Query):
    global faiss_index

    prefix = f"rag-project-bucket-01/test-output/{data.org}/{data.dept}"
    ...
    if retrieved:
        ...
        return {"result": "No Relevant Document Found", "source_documents": []}     #If no relevent documents found sends a hardcoded answer

#Gets the documents from which the answer was genrated
@app.get("/get_source_document/{doc_id}")
def get_source_document(doc_id: str, org: str, dept: str, base_output_dir: str = "rag-project-bucket-01/test-output"):
...
def get_source_document_url(doc_id: str, org: str, dept: str, base_output_dir: str = "rag-project-bucket-01/test-output"):
...
def delete_document(org: str, dept: str, doc_id: str, base_output_dir: str = "rag-project-bucket-01/test-output"):
